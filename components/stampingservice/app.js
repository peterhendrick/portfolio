...
.controller('MyCtrl', ['$scope', 'Upload', '$http', '$interval',
    function($scope, Upload, $http, $interval) {
      ...
      function isFileInBlockchain(fileHashString) {
        ...
        $http.get(bitcoreServiceBasePath + '/hash/' +
                fileHashString)
            .success(gotFile)
            .error(didNotGetFile);
        ...
        function gotFile(data, statusCode) {
          ...
          $scope.previousTimestamps = $scope.previousTimestamps.map(
                    function(ts) {
        ...
        function didNotGetFile(data, statusCode) {
          if (statusCode === 404) {
            if (pendingFileHashes[fileHash]) {
              $scope.pendingTimestamp = pendingFileHashes[
                  fileHash];
            }
          }
        }
      ...
      $scope.stampFile = function() {
        ...
        $scope.address = new bitcore.Address(publicKey, bitcore
            .Networks.testnet).toString();
      ...
      function monitorAddress(address, cb) {
        ...
        pollInterval = $interval(function() {
          console.log(
              'monitorAddress interval called for address:',
              address);
          $http.get(bitcoreServiceBasePath + '/address/' +
                  address)
      ...
      function timeStampFile(unspentOutput, privateKey) {
        ...
        var unspent2 = new UnspentOutput(unspentOutput);
